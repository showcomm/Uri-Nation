...
- **Pee = water. Modifiers = intent.** Pee disks are purchased with water only. What you DO with the pee is determined by the modifier token you spend with it.
...
Bladder capacity. Water is what makes pee — large pee costs 2 water, small pee costs 1 water. The universal fuel.
...
Pee disks are purchased with water only. They are pure urine — no modifiers needed to create them.

**Large pee disk** — Costs **2 water pegs** to create. Puddle-shaped disk in player colour. Used for Claims and Challenges on territory spaces.

**Small pee disk** — Costs **1 water peg** to create. Smaller puddle-shaped disk in player colour. Used for Fortify, Ally, and Break Alliance actions. Also used as compulsory bond markers (provided from the bank when passing an active bond space, costs 1 water).
...
- **Peg holes around the rim** — for water pegs (blue) only. Represent bladder capacity. Remove pegs when creating pee disks (2 for large, 1 for small). Refill by drinking or passing home.
- **Centre dish** — holds all modifier tokens (swagger bones, drive sticks, grit tokens, bond hearts) and stockpiled pee disks (large and small).
...
All actions are built from pee disks + modifier tokens. Water creates pee. Modifiers determine intent.
...
1. Attacker pays: 1 large pee disk (2 water) + 1 Drive token.
2. Both roll d6. Apply modifiers.
3. **Attacker wins:** Both large pees destroyed. Fortify tokens removed. Territory empty. Attacker may **re-claim** by spending 1 swagger OR 1 grit.
...
Same roll but targeting a bond space. Attacker pays large pee (2 water) + 1 Drive.
...
**Recipe:** Large pee (2 water) + 1 swagger. (Bruiser: no swagger.)

Place on an empty territory adjacent to your path position.

### 10.2 Challenge

**Recipe:** Large pee (2 water) + 1 drive.

See Section 9 for full resolution.

### 10.3 Fortify

**Recipe:** Small pee (1 water) + 1 grit. (Yapper: no grit.)

Place on your claimed territory. Max 3 per territory. Each +1 to defender's roll.

### 10.4 Ally (Initiate)

**Recipe:** Small pee (1 water) + 1 bond. (Diplomat: no bond.)
...
**Cost:** 1 water. Bank provides small pee in your colour.
...
Destroy entire bond stack. All markers returned to bank. Any bonded player can break it. Game of chicken — who pays the swagger?
...
- Small pee disks (smaller puddle-shaped) in 4 player colours + bank supply for compulsory joins
...
- Pee disk water costs (large=2, small=1): right ratio?
